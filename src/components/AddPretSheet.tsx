"use client";

import { useState } from "react";
import { ArrowDownLeft, ArrowUpRight, X } from "lucide-react";
import { colors, glow } from "@/lib/theme";
import { newClientId } from "@/lib/sync";
import type { Account, Ami, NewPretInput, PretDirection } from "@/lib/types";

const inputStyle: React.CSSProperties = {
  width: "100%",
  boxSizing: "border-box",
  background: "linear-gradient(155deg, oklch(1 0 0 / 0.07), oklch(1 0 0 / 0.02))",
  border: "1px solid oklch(1 0 0 / 0.12)",
  WebkitBackdropFilter: "blur(18px) saturate(160%)",
  backdropFilter: "blur(18px) saturate(160%)",
  borderRadius: 12,
  padding: "12px 14px",
  color: colors.textSecondary,
  fontSize: 14,
  fontFamily: "inherit",
  marginBottom: 18,
};

export function AddPretSheet({
  ami,
  accounts,
  initialDirection,
  onClose,
  onConfirm,
}: {
  ami: Ami;
  accounts: Account[];
  initialDirection: PretDirection;
  onClose: () => void;
  onConfirm: (input: NewPretInput) => Promise<void> | void;
}) {
  const [direction, setDirection] = useState<PretDirection>(initialDirection);
  const [montant, setMontant] = useState("");
  const [note, setNote] = useState("");
  const [compteId, setCompteId] = useState<string | null>(accounts[0]?.id ?? null);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const canSave = parseFloat(montant) > 0 && !saving;

  async function save() {
    if (!canSave) return;
    setSaving(true);
    setError(null);
    try {
      await onConfirm({
        clientId: newClientId(),
        amiId: ami.id,
        direction,
        montant: parseFloat(montant),
        note: note.trim() || null,
        compteId,
        date: new Date().toISOString(),
      });
    } catch (e) {
      setError(e instanceof Error ? e.message : "Impossible d'enregistrer le mouvement");
      setSaving(false);
    }
  }

  return (
    <>
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "oklch(0 0 0 / 0.5)", backdropFilter: "blur(2px)", zIndex: 20, animation: "backdropIn 0.22s ease" }} />
      <div
        className="glass-sheet"
        style={{
          position: "absolute",
          left: 0,
          right: 0,
          bottom: 0,
          animation: "sheetUp 0.32s cubic-bezier(0.32,0.72,0,1)",
          borderRadius: "26px 26px 0 0",
          zIndex: 21,
          boxSizing: "border-box",
          padding: "14px 18px calc(env(safe-area-inset-bottom, 0px) + 28px)",
        }}
      >
        <div style={{ width: 36, height: 4, borderRadius: 100, background: colors.white15, margin: "0 auto 16px" }} />

        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: colors.textPrimary }}>Mouvement avec {ami.nom}</div>
          <div
            onClick={onClose}
            className="tap glass"
            style={{ width: 28, height: 28, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: colors.textMuted, cursor: "pointer" }}
          >
            <X size={14} />
          </div>
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
          {(
            [
              { d: "donne" as PretDirection, label: "J'ai donné", Icon: ArrowUpRight, accent: colors.accentGold },
              { d: "recu" as PretDirection, label: "J'ai reçu", Icon: ArrowDownLeft, accent: colors.accentGreen },
            ]
          ).map(({ d, label, Icon, accent }) => (
            <div
              key={d}
              onClick={() => setDirection(d)}
              style={{
                flex: 1,
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                gap: 6,
                padding: "11px 0",
                borderRadius: 12,
                fontSize: 13,
                fontWeight: 700,
                cursor: "pointer",
                background: direction === d ? colors.white8 : colors.white4,
                color: direction === d ? colors.textPrimary : colors.textFaint,
                border: `1px solid ${direction === d ? accent : "transparent"}`,
              }}
            >
              <Icon size={15} color={accent} />
              {label}
            </div>
          ))}
        </div>

        <div style={{ fontSize: 11.5, color: colors.textFaint, marginBottom: 18, lineHeight: 1.5 }}>
          {direction === "donne"
            ? "Prêt à un ami ou remboursement de ce que vous lui deviez."
            : "Remboursement d'un ami ou argent qu'il vous prête."}
        </div>

        <div style={{ fontSize: 12.5, fontWeight: 700, color: colors.textMuted, marginBottom: 8 }}>MONTANT (MRU)</div>
        <input
          value={montant}
          onChange={(e) => setMontant(e.target.value.replace(/[^0-9.]/g, ""))}
          placeholder="0"
          inputMode="decimal"
          style={inputStyle}
        />

        <div style={{ fontSize: 12.5, fontWeight: 700, color: colors.textMuted, marginBottom: 8 }}>COMPTE</div>
        <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
          {accounts.map((a) => (
            <div
              key={a.id}
              onClick={() => setCompteId(a.id)}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 7,
                padding: "8px 12px",
                borderRadius: 100,
                fontSize: 12,
                fontWeight: 600,
                cursor: "pointer",
                background: compteId === a.id ? colors.white8 : colors.white4,
                color: compteId === a.id ? colors.textPrimary : colors.textFaint,
                border: `1px solid ${compteId === a.id ? a.couleur : "transparent"}`,
              }}
            >
              <div style={{ width: 8, height: 8, borderRadius: 2, background: a.couleur }} />
              {a.nom}
            </div>
          ))}
          <div
            onClick={() => setCompteId(null)}
            style={{
              padding: "8px 12px",
              borderRadius: 100,
              fontSize: 12,
              fontWeight: 600,
              cursor: "pointer",
              background: compteId === null ? colors.white8 : colors.white4,
              color: compteId === null ? colors.textPrimary : colors.textFaint,
              border: `1px solid ${compteId === null ? colors.white15 : "transparent"}`,
            }}
          >
            Hors compte
          </div>
        </div>
        {compteId === null && (
          <div style={{ fontSize: 11, color: colors.textFaint, marginTop: -10, marginBottom: 18 }}>
            « Hors compte » : le solde de vos comptes ne bougera pas, seul le solde de l&apos;ami est mis à jour.
          </div>
        )}

        <div style={{ fontSize: 12.5, fontWeight: 700, color: colors.textMuted, marginBottom: 8 }}>NOTE (OPTIONNEL)</div>
        <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Ex : courses, taxi…" style={{ ...inputStyle, marginBottom: 22 }} />

        {error && <div style={{ fontSize: 12, color: colors.accentRed, marginBottom: 12 }}>{error}</div>}

        <div
          onClick={save}
          className="tap"
          style={{
            textAlign: "center",
            padding: 14,
            borderRadius: 14,
            fontSize: 14,
            fontWeight: 700,
            cursor: canSave ? "pointer" : "default",
            background: canSave ? (direction === "recu" ? colors.accentGreen : colors.accentGold) : colors.white8,
            color: canSave ? colors.neutralIcon : colors.textFaint,
            boxShadow: canSave ? glow(direction === "recu" ? colors.accentGreen : colors.accentGold, 0.4) : "none",
          }}
        >
          {saving ? "Enregistrement…" : "Enregistrer"}
        </div>
      </div>
    </>
  );
}
